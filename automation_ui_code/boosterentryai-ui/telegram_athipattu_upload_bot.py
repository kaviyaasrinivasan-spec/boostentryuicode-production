...
# ---------- Configuration ----------
TELEGRAM_BOT_TOKEN = "8645852222:AAGBep7AL89rAbCw9BNFvFXmMIyu-RIsEfs" 
TELEGRAM_API_URL = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}"

# Athipattu Specific Configuration
CLIENT_ID = 6
DOC_FORMAT_ID = 6
...
def upload_to_backend(raw_bytes, filename, mime_type):
    try:
        ...
        data = {
            ...
            "uploaded_by": "TelegramBot_Athipattu"
        }
...
def handle_document(msg):
    ...
    if not filename.lower().endswith(('.pdf', '.jpg', '.jpeg', '.png', '.webp', '.tiff', '.bmp')):
        ...
        return
    requests.post(f"{TELEGRAM_API_URL}/sendMessage", json={"chat_id": chat_id, "text": f"⏳ Processing: {filename}..."})
    file_id = doc['file_id']
    ...
    success, msg_resp = upload_to_backend(file_data, filename, doc.get('mime_type', 'application/octet-stream'))
...
def main():
    print(f"🤖 Athipattu Bot started in current directory!")
    offset = None
    while True:
        try:
            ...
            if resp.get('ok') and resp.get('result'):
                for update in resp['result']:
                    ...
                    if 'message' in update:
                        m = update['message']
                        if 'document' in m: handle_document(m)
                        elif 'text' in m and m['text'] == '/start':
                            requests.post(f"{TELEGRAM_API_URL}/sendMessage", json={"chat_id": m['chat']['id'], "text": "👋 Welcome to Athipattu Upload Bot!"})
        except Exception as e:
            print(f"Error: {e}")
            time.sleep(5)
