...
def insert_into_doc_log(payload: DocLogIn,
                        ...
                        doc_format_id: Optional[int] = None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        # parse filename and find client/docformat ONLY IF not provided
        if client_id is None or doc_format_id is None:
            client_tok, doc_type_tok = parse_doc_name(payload.doc_name)
            parsed_client_id, parsed_doc_format_id = find_client_and_docformat(cur, client_tok, doc_type_tok)
            
            if client_id is None:
                client_id = parsed_client_id
            if doc_format_id is None:
                doc_format_id = parsed_doc_format_id
            ...
            logger.info("Using provided IDs: client_id=%s doc_format_id=%s for %s", 
                        client_id, doc_format_id, payload.doc_name)
...
@app.get("/pod-monitoring")
async def get_pod_monitoring(from_date: str = None, to_date: str = None, status: str = "all"):
    conn = None
    try:
        conn = get_conn()
        ...
        if from_date:
            date_filter = f"'{from_date}'::date"

        # Get Stats
        cur.execute(f"SELECT COUNT(*) as total, COUNT(CASE WHEN di_no IS NOT NULL THEN 1 END) as completed, COUNT(CASE WHEN di_no IS NULL THEN 1 END) as failed FROM boostentryai.pod_upload WHERE created_at::date = {date_filter}")
        stats = cur.fetchone()

        # Get Items
        cur.execute(f"SELECT id, di_no, created_at, seal_invoice FROM boostentryai.pod_upload WHERE created_at::date = {date_filter} ORDER BY created_at DESC")
        ...
        for r in rows:
            items.append({
                "id": r['id'],
                "di_no": r['di_no'] or "PENDING",
                "created_at": r['created_at'].isoformat() if r['created_at'] else None,
                "status": "Completed" if r['di_no'] else "Failed"
            })
...
async def insert_with_file(doc_name: str = Form(...),
                           ...
                           doc_format_id: Optional[int] = Form(None),
    ...
    if file_bytes is not None and fsize > 0:
        try:
            ...
            # DISABLED: Normal invoices should NOT be copied to POD pipeline
            # try:
            #     pod_pdfs_path = "/root/wbai_doc_extractor_engine-maincopy/pod_pdfs"
            #     os.makedirs(pod_pdfs_path, exist_ok=True)
            #     shutil.copy2(file_full_path, os.path.join(pod_pdfs_path, filename))
            #     logger.info(f"✅ Successfully COPIED to extraction engine folder: {pod_pdfs_path}")
            # except Exception as e:
            #     logger.warning(f"⚠️ Copy to pod_pdfs failed: {e}")
    ...
    result = insert_into_doc_log(payload, client_id=client_id, doc_format_id=doc_format_id,
