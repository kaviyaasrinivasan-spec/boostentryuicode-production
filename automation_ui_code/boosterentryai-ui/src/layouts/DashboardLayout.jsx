...
import { LogOut, LayoutDashboard, Upload, Monitor, ClipboardCheck, Menu, ArrowLeftRight, Truck, FileStack, ScanSearch, PieChart } from "lucide-react";
    ...
    <div className="flex min-h-screen bg-gray-50 overflow-x-hidden">
      ...
      <div className={`fixed inset-0 z-40 md:hidden transition-opacity ${sidebarOpen ? "block" : "pointer-events-none"}`}>
        ...
        <aside className={`absolute left-0 top-0 bottom-0 w-72 bg-[#3B29D9] text-white p-4 transform transition-transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
          ...
          <nav className="mt-4 space-y-1">
            ...
            <NavLink to="/pod-upload" onClick={() => setSidebarOpen(false)} className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-[15px] ${isActive ? "bg-[#5243F3] text-white font-semibold" : "text-indigo-100 hover:bg-[#5243F3] hover:text-white"}`}>
              <FileStack size={18} /> POD Upload
            </NavLink>

            <NavLink to="/pod-monitoring" onClick={() => setSidebarOpen(false)} className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-[15px] ${isActive ? "bg-[#5243F3] text-white font-semibold" : "text-indigo-100 hover:bg-[#5243F3] hover:text-white"}`}>
              <ScanSearch size={18} /> POD Monitoring
            </NavLink>

            <NavLink to="/invoice-report" onClick={() => setSidebarOpen(false)} className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-[15px] ${isActive ? "bg-[#5243F3] text-white font-semibold" : "text-indigo-100 hover:bg-[#5243F3] hover:text-white"}`}>
              <PieChart size={18} /> Invoice Report
            </NavLink>
          </nav>
        </aside>
      </div>
      ...
      <aside className="hidden md:flex w-64 flex-shrink-0 bg-[#3B29D9] text-white flex-col">
        ...
        <nav className="flex flex-col space-y-1 px-3 mt-3">
          ...
          <NavLink to="/pod-upload" className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-[15px] transition ${isActive ? "bg-[#5243F3] text-white font-semibold" : "text-indigo-100 hover:bg-[#5243F3] hover:text-white"}`}>
            <FileStack size={18} /> POD Upload
          </NavLink>

          <NavLink to="/pod-monitoring" className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-[15px] transition ${isActive ? "bg-[#5243F3] text-white font-semibold" : "text-indigo-100 hover:bg-[#5243F3] hover:text-white"}`}>
            <ScanSearch size={18} /> POD Monitoring
          </NavLink>

          <NavLink to="/invoice-report" className={({ isActive }) => `flex items-center gap-3 px-3 py-2 rounded-lg text-[15px] transition ${isActive ? "bg-[#5243F3] text-white font-semibold" : "text-indigo-100 hover:bg-[#5243F3] hover:text-white"}`}>
            <PieChart size={18} /> Invoice Report
          </NavLink>
        </nav>
      </aside>
