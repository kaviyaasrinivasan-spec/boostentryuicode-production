  ...
  useEffect(() => {
    const fetchDoc = async () => {
      try {
        ...
        // ✅ Filter against a preferred order of fields to match Human Review
        ...
        const filteredData = preferredFields
          .map(f => {
            const found = extracted.find(item => {
              const norm1 = String(item.field || "").replace(/[^A-Za-z0-9]/gi, "").toLowerCase();
              const norm2 = f.replace(/[^A-Za-z0-9]/gi, "").toLowerCase();
              return norm1 === norm2;
            });
            // Force include it even if not in response, so the user sees an empty box at least
            return { field: f, value: found ? (found.value || "") : "" };
          });
          // We removed .filter(Boolean) because we want to see the boxes even if empty
          // but maybe we only want to force certain fields. 
          // For now, let's keep it simple: show all preferred fields.
