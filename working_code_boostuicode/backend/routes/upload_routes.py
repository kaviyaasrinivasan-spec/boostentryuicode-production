...
def convert_image_to_pdf_bytes(raw_bytes: bytes,
    ...
    """
    Convert raw image bytes -> single-page PDF bytes while:
      - opening large images safely (temporarily disabling MAX_IMAGE_PIXELS)
      - resizing if pixel count > max_pixels
      - iteratively reducing image dimensions (downscale) until PDF size <= target_bytes or scale <= min_scale
    ...
    try:
        ...
        # Resize if pixel count exceeds cap
        ...
        def save_pdf(image_obj, resolution: float = 72.0) -> bytes:
            ...
            try:
                ...
                # fallback: save as JPEG and embed as PDF
                tmp = io.BytesIO()
        ...
        # If resulting PDF > target_bytes, iteratively downscale
        if target_bytes and len(pdf_bytes) > target_bytes:
            attempt_img = img
            attempt_scale = current_scale
            # Loop: reduce by a conservative factor until under target or reach min_scale
            while len(pdf_bytes) > target_bytes and attempt_scale > min_scale:
                # compute scale step (conservative)
